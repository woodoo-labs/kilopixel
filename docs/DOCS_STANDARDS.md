...
* **Terminology & Formatting (The "Code vs. Prose Rule"):**
  ...
  * **Inline Callout/List Headers:** Bold prefixes at the beginning of paragraphs, bulleted lists, or callout bodies (e.g., `<li><strong>Concept Name:</strong> ...</li>`) MUST use plain English in Title Case with the colon placed *inside* the `<strong>` tag. NEVER wrap `<code>` tags inside the bold lead-in; weave technical attribute or keyword names naturally into the body sentence immediately following the title.
...
* **Toggle Button Implementations:** Toggle button `onclick` handlers must follow **Option 1 (Explicit Native DOM + UI Helper)**: put the native `document.getElementById('id').setAttribute('attr', 'value')` call explicitly first so developers see transparently how Kilopixel works, then call `pxlDocs.updateToggle(this, null, 'codeId', 'value')` to handle UI button state, code mark updates, and 1-second pulse flashing:
  ```html
  <button class="toggle-btn active" onclick="document.getElementById('sec1Circle').setAttribute('pie', 'false'); pxlDocs.updateToggle(this, null, 'sec1CirclePieCode', 'false');">false (Open Arc)</button>
  ```
  * **No Value Readout in Label (Buttons & Dropdowns):** Unlike sliders (which have no text readout on their track), toggle button groups (`.toggle-group`) and dropdown menus (`<select>`) explicitly display their selected state via the highlighted `.active` button or active `<option>`. Therefore, `<label>` elements for both toggle buttons and dropdowns MUST omit `<span id="...Val">` readouts to avoid redundant UI clutter. For toggle buttons, pass `null` as the `lblId` argument in `pxlDocs.updateToggle`.
